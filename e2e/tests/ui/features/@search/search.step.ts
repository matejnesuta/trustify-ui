...
Then('a total number of {int} {string} should be visible in the tab', async ({page},count: number, arg: string) => {
...
Then('the results should be limited to {int} suggestions', async ({page}, arg: number) => {
  const searchPage = new SearchPage(page);
  expect(await searchPage.totalAutoFillResults()).toBeLessThanOrEqual(arg*4);
